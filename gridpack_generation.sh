    ...
    case ":$current_value:" in
        ...
        *)
            if [ -n "$current_value" ]; then
                eval "export $var_name=\"$new_path:$current_value\""
            else
...
setup_runtime_library_paths () {
    local mg_root="$1"
    local ninja_lib=""
    local gfortran_lib=""
    local stdcpp_lib=""

    if [ -d "$mg_root/HEPTools/ninja/lib" ]; then
        ninja_lib="$mg_root/HEPTools/ninja/lib"
    elif [ -d "$mg_root/HEPTools/lib" ]; then
        ninja_lib="$mg_root/HEPTools/lib"
    fi

    gfortran_lib=$(dirname "$(gfortran -print-file-name=libgfortran.so 2>/dev/null)")
    stdcpp_lib=$(dirname "$(g++ -print-file-name=libstdc++.so 2>/dev/null)")

    prepend_env_path LD_LIBRARY_PATH "$ninja_lib"
    prepend_env_path LIBRARY_PATH "$ninja_lib"
    prepend_env_path LD_RUN_PATH "$ninja_lib"

    prepend_env_path LD_LIBRARY_PATH "$gfortran_lib"
    prepend_env_path LIBRARY_PATH "$gfortran_lib"
    prepend_env_path LD_RUN_PATH "$gfortran_lib"

    prepend_env_path LD_LIBRARY_PATH "$stdcpp_lib"
    prepend_env_path LIBRARY_PATH "$stdcpp_lib"
...
make_gridpack () {
    ...
    LOCAL_MG_SOURCE=${PRODHOME}/MG5_aMC_v2_6_5
    USE_LOCAL_MG=0
    ...
    if [ ! -d ${GEN_FOLDER}/${name}_gridpack ]; then
      ...
      #############################################
      if [ -d "${LOCAL_MG_SOURCE}" ]; then
        echo "Using local MadGraph source from ${LOCAL_MG_SOURCE}"
        cp -a "${LOCAL_MG_SOURCE}" "${MGBASEDIRORIG}"
        USE_LOCAL_MG=1
      else
      ...
      cd $MGBASEDIRORIG
      mkdir -p HEPTools/lib HEPTools/include HEPTools/bin
    ...
    #  echo "set output_dependencies internal" >> mgconfigscript
      ...
      if [ -f "$WORKDIR/$MGBASEDIRORIG/HEPTools/ninja/lib/libninja.a" ]; then
        echo "set ninja $WORKDIR/$MGBASEDIRORIG/HEPTools/ninja/lib" >> mgconfigscript
      elif [ -f "$WORKDIR/$MGBASEDIRORIG/HEPTools/lib/libninja.a" ]; then
        echo "set ninja $WORKDIR/$MGBASEDIRORIG/HEPTools/lib" >> mgconfigscript
      fi
